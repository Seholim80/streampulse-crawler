...
interface SoopRealBroad {
  ...
  broad_no: string;
  broad_title: string;
  category_name: string;       // 실제 필드명! (broad_cate_name 아님)
...
export async function collectSoop(): Promise<{
  ...
  if (rawBroads.length > 0) {
    ...
    console.log(`  🔍 샘플 데이터:`, JSON.stringify({
      ...
      category_name: (sample as any).category_name,
      category_tags: (sample as any).category_tags,
      broad_cate_no: (sample as any).broad_cate_no,
